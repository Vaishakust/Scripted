     ...
     //   final int jwtExpiryWindowSeconds = 60 * 3;
        final JwtAuthorizationGenerator jwtAuthorisationGenerator = new JwtAuthorizationGenerator(jwtWriterFactory, jwtExpiryWindowSeconds) {
            ...
            public Option<String> generate(HttpMethod httpMethod, URI url, Map<String, List<String>> parameters, AcHost acHost,
                ...
                return Option.some(JWT_AUTH_HEADER_PREFIX + encodeJwtvalue(httpMethod, url, paramsAsArrays, userId.getOrNull(),
                        acHost));
            }

          @SuppressWarnings("unused")
		private String encodeJwtvalue(HttpMethod httpMethod, URI targetPath, Map<String, String[]> params, String userKeyValue,
                ...
                try {
                    if (!StringUtils.isEmpty(targetPath.getQuery())) {
                        ...
                        completeParams.putAll(constructParameterMapCollection(targetPath));
                    }
            ...
            private Map<String, String[]> constructParameterMapCollection(URI uri) throws UnsupportedEncodingException {
                ...
                while (!cursor.atEnd()) {
                    ...
                    if (!StringUtils.isEmpty(nameValuePair.getName())) {
                        String decodedName = urlDecodevalue(nameValuePair.getName());
                        String decodedValue = urlDecodevalue(nameValuePair.getValue());
            ...
            private String urlDecodevalue(final String content) throws UnsupportedEncodingException , NullPointerException {
        ...
        if(jwt != null) {
        authorizationHeaderValue = jwt.getOrNull();
        }
