...
public class SelfHealingUtil {
	public static Logger LOGGER = LogManager.getLogger(SelfHealingUtil.class);
	...
	public static void webWaitForPresence(int time, WebElement locator, WebDriver driver) {
		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(time), Duration.ofSeconds(10));
	...
	public  void mobWaitForPresence(WebElement ele, int time, WebDriver mobDriver) {
		try {
			By byEle = mobileElementBy(ele);
			WebDriverWait wait = new WebDriverWait(mobDriver, Duration.ofSeconds(time), Duration.ofSeconds(10));
			wait.until(ExpectedConditions.presenceOfElementLocated(byEle));
	...
	public static By mobileElementBy(WebElement mobileElement) {
